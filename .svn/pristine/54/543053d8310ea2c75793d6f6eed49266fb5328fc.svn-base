...
public class Neurons extends PApplet implements PeasyWheelHandler{
	int count = 0;
	float count2 = 0;
	
	int num = 3000;
	float RADIUS = 36;
	float SCALE = (float) 250.0;
	...
	class Neuron{
		...
		void draw(){
			...
			stroke(lerpColor(200, 255, norm(val,0,255)),20);
			drawConnections();
		...
		void step(){
			...
			if(dist(x,y,z,n[base.B].xx,n[base.B].yy,n[base.B].zz)<1.0){
				...
				if(deadcount<0){
					...
					if(n[base.A].id!=1){
						n[base.A].xx+=((n[base.B].x-n[base.A].x)/1.1)*noise((float) ((frameCount+n[base.A].id)/18.0));;
						n[base.A].yy+=((n[base.B].y-n[base.A].y)/1.1)*noise((float) ((frameCount+n[base.A].id)/17.0));
						n[base.A].zz+=((n[base.B].z-n[base.A].z)/1.1)*noise((float) ((frameCount+n[base.A].id)/16.0));



						n[base.A].xx-=((n[base.B].x-n[base.A].x)/1.1)*noise((float) ((frameCount+n[base.B].id)/18.2));;
						n[base.A].yy-=((n[base.B].y-n[base.A].y)/1.1)*noise((float) ((frameCount+n[base.B].id)/17.2));
						n[base.A].zz-=((n[base.B].z-n[base.A].z)/1.1)*noise((float) ((frameCount+n[base.B].id)/16.2));
					}
	...
	@Override
	public void handleWheel(int arg0) {
